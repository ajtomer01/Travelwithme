...
// let me paste the array named dot
  ...
  return (
    <section className='main container section'>
      ...
      <div className="secTitle">
        <h3  data-aos="fade-right"  className="titl">
          most visited destinations
        </h3>
      </div>

      <div className="secContent grid">
        {/* we are going to use high ordfer array method(map).To do that we shall use a list of object in one array. Create an array named data and from that wwe shall .map() array to fetch each desitnation at once */}

        {
          Data.map(({id, imgSrc, destTitle, location, grade, fees, description })=>{
            return(
              <div key={id} data-aos="fade-up"  className="singleDestination">
                {/* it will return single id fro, the map array */}
                ...
                <div className="cardInfo">
                  ...
                  <button className='btn flex'>
                    DETATILS <HiOutlineClipboardCheck className="icon" />
                  </button>
                </div>
              </div>
            )
